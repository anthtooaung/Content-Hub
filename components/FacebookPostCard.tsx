    ...
    >
      ...
      {images.length > 0 ? (
        ...
        >
          ...
          <span className="text-[14px] font-medium text-[#65676B]">Add an image to this post</span>
        </div>
      )}
      ...
      <div className="flex items-center justify-between px-4 py-2.5 text-[15px] text-[#65676B]">
        <div className="flex items-center gap-1">
          <div className="flex -space-x-1">
            <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[#1877F2] text-[12px]">👍</span>
            <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[#F33E58] text-[12px]">❤️</span>
          </div>
      ...
      <div className="px-4 py-3">
        <div className="mb-3 flex items-center gap-2">
          <div className="h-8 w-8 rounded-full bg-[#E4E6EB] flex-shrink-0" />
          <input
            placeholder="Write a comment…"
            readOnly
            className="flex-1 rounded-full bg-[#F0F2F5] px-3 py-2 text-[15px] text-[#65676B] outline-none"
          />
        </div>
        <div className="flex gap-2">
          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-[#A78BFA] to-[#7C3AED] text-[12px] font-bold text-white flex-shrink-0">
            S
          </div>
          <div>
            <div className="rounded-[18px] bg-[#F0F2F5] px-3 py-2">
              <div className="text-[13px] font-semibold">Sarah M.</div>
              <div className="text-[15px] leading-[1.4]">OMG the mango iced latte is incredible! 🥭☕</div>
            </div>
            <div className="mt-1 flex gap-3 pl-3 text-[12px] font-bold text-[#65676B]">
        ...
        {content?.score && (
          <div className="mt-3 pt-3 border-t border-[#E4E6EB]">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="text-[13px] font-medium text-[#050505]">Score:</span>
                <span className="text-[18px] font-bold text-[#050505]">{content.score.overall}/100</span>
            ...
            {showScoreDetails && (
              <div className="mt-3 space-y-2">
                ...
                {content.score.suggestions?.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-[#E4E6EB]">
                    <p className="text-[11px] font-medium text-[#65676B] mb-2">Suggestions:</p>
                    <ul className="space-y-1">
                      {content.score.suggestions.map((s: string, i: number) => (
                        <li key={i} className="text-[11px] text-[#65676B] flex items-start gap-1.5">
...
function ScoreBar({ label, score }: { label: string; score: number }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span className="text-[11px] font-medium text-[#050505] w-6 text-right">{score}</span>
    </div>
  );
}
