...
function getPasswordStrength(pw: string): { score: number; label: string; color: string } {
  ...
  if (score === 2) return { score: 2, label: 'Medium', color: 'bg-warning' };
    ...
    <div className="flex min-h-screen">
      ...
      <div className="flex flex-1 items-center justify-center p-12 max-lg:p-8">
        <div className="w-full max-w-[380px]">
          ...
          <h1 className="mb-1 text-[24px] font-bold text-text-primary">Create your account</h1>
          <p className="mb-7 text-sm text-text-muted">Start generating content in seconds</p>

          <div className="flex gap-2.5 mb-5">
            <button
              type="button"
              onClick={() => signIn('google', { callbackUrl: '/generate' })}
              className="flex-1 flex items-center justify-center gap-2 rounded-control border border-primary-300 bg-primary-50 px-4 py-2.5 text-[13px] font-medium text-primary transition-all hover:-translate-y-0.5 hover:bg-primary hover:text-white hover:border-primary hover:shadow-[0_4px_12px_rgba(37,99,235,0.3)]"
            >
          ...
          <div className="flex items-center gap-3 mb-5">
            ...
            <span className="text-xs text-text-muted whitespace-nowrap">or sign up with email</span>
            <div className="flex-1 h-px bg-border" />
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="mb-1.5 block text-[13px] font-medium text-text-secondary">First name</label>
                <input
                  type="text"
                  placeholder="Jane"
                  required
              ...
              <div>
                <label className="mb-1.5 block text-[13px] font-medium text-text-secondary">Last name</label>
                <input
                  type="text"
                  placeholder="Smith"
                  required
            ...
            <div className="flex items-center gap-2 text-[13px] text-text-secondary cursor-pointer">
            ...
            <button
              type="submit"
              disabled={loading}
              className="h-11 w-full rounded-control bg-primary text-sm font-semibold text-white transition-all hover:-translate-y-0.5 hover:bg-primary-600 hover:shadow-[0_4px_12px_rgba(37,99,235,0.3)] disabled:pointer-events-none disabled:opacity-70"
            >
              {loading ? (
